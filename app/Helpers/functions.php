...
{
    // Önce Dotenv ile yüklenmiş ortam değişkenlerine bak (en güvenilir kaynak)
    $env = $_ENV[$key] ?? getenv($key);
    if ($env !== false && $env !== null) {
        return $env;
    }

    // Aksi halde proje kökündeki .env dosyasını oku
    $envPath = (defined('BASE_PATH') ? BASE_PATH : dirname(__DIR__, 2)) . '/.env';
